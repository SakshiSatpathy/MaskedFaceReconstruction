...
def fit(args):
    ...
    with torch.no_grad():
        ...
        pred_dict = recon_model(coeffs, render=True, finalSet=True)
        rendered_img = pred_dict['rendered_img']
        ...
        """
        ###### ADDING CODE HERE FOR TEXTURES ######
        #texture_img = pred_dict['face_texture']
        arr = pred_dict['rendered_img'].cpu().numpy()
        print(f"texture image original size: {arr.shape}")
        rgb = arr[:,:, :3]
        print(f"Converted texture image size: {rgb.shape}")
        converted_texture_img = rgb.astype(np.uint8)
        out_composed_texture_path = os.path.join(
            args.res_folder, basename + '_texture_img.jpeg')
        print(out_composed_img_path)
        
        if not cv2.imwrite(out_composed_texture_path, out_mask):
            print("Could not write image")
        else:
            cv2.imwrite(out_composed_texture_path, out_mask)
        ############################################
        """
