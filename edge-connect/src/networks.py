...
class EdgeGenerator(BaseNetwork):
    ...
    def forward(self, x):
        inpt = x
        x = self.encoder(x)
        print(f"From networks.py; Output of Encoder shape: {x.shape}")
        x = self.middle(x)
        print(f"From networks.py; Output of Middle shape: {x.shape}")
        x = self.decoder(x)
        print(f"From networks.py; Output of Decoder shape: {x.shape}")
        x = torch.sigmoid(x)
        x = output_align(inpt, x)
        print(f"From networks.py; Output of Output_Align shape: {x.shape}")
        return x
