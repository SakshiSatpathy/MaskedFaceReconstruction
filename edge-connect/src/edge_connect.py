    ...
    def test(self):
        ...
        for items in test_loader:
            ...
            # inpaint with edge model / joint model
                ...
                print(f"Line 326, size of images_gray, {images_gray.shape}, size of edges: {edges.shape}, size of masks: {masks.shape}")
                edges = self.edge_model(images_gray, edges, masks).detach() # For some reason, the edge_model is changing the dimension of the edges.
                print(f"Line 327, size of images, {images.shape}, size of edges: {edges.shape}, size of masks: {masks.shape}")
                outputs = self.inpaint_model(images, edges, masks)
                outputs_merged = (outputs * masks) + (images * (1 - masks))
